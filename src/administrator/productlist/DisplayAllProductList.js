...
export default function DisplayAllProductList(){
  ...
  const [picture, setPicture]=useState({file:'/assets/shopping-cart1.png',bytes:''})
  ...
  const fillProduct=()=>{
   ...
   ////////////////////////
    ...
    const validation=()=>{
      ...
      if(!qty)
      {
        handleError("qty","Plz Input Weight")
        isValid=false
      }
      if(!stock)
      {
    ...
    const handleEditData=async()=>{
      ...
      if(validation())
      {
         var body={productlistid:productListId, productlistname:productListName, productid:productId, categoryid:categoryId, subcategoryid:SubCategoryId, description:Description, rate:rate, offer:offer, qty:qty, stock:stock, status:status}
...
borderRadius: 10}}>
    ...
    <Grid item xs={6}>
      <FormControl fullWidth>
        ...
        <Grid item xs={6}>
          <TextField value={rate} error={error.rate?true:false} helperText={error.rate} onFocus={()=>handleError("rate", null)} onChange={(event) => { setRate(event.target.value) }} label=" Rate" variant="outlined" fullWidth />
        </Grid>
        <Grid item xs={6}>
          <TextField value={offer} error={error.offer?true:false} helperText={error.offer} onFocus={()=>handleError("offer", null)} onChange={(event) => { setOffer(event.target.value) }} label=" Offer Price" variant="outlined" fullWidth />
        </Grid>
        <Grid item xs={6}>
          <TextField value={qty} error={error.qty?true:false} helperText={error.qty} onFocus={()=>handleError("qty", null)} onChange={(event) => { setQty(event.target.value) }} label=" Quantity" variant="outlined" fullWidth />
        </Grid>
        <Grid item xs={6}>
