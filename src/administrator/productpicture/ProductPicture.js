...
const fetchAllProductList=async(productId)=>{
    var result=await postData('productpicture/productlist_by_productid',{productid:productId})
...
const fillProductList=()=>{
    return productsList.map((item)=>{
        return <MenuItem value={item.productlistid}>{item.productlistname} {item.qty}</MenuItem>
    })
}
...
const handleSubmit=async()=>{
    ...
    var result=await postData('productpicture/submit_product_pictures',formData)
    if(result.status)
            {
                Swal.fire({
                    icon: 'success',
